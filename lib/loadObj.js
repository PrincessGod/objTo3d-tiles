...
function loadObj(objPath, options) {
    ...
    function isConvex(positions2D) {
        ...
        var currentTurnDirection = getTurnDirection(positions2D[length-2], positions2D[length-1], positions2D[0]);
        if (turnDirection * currentTurnDirection < 0) {
	        return false;
        }
        currentTurnDirection = getTurnDirection(positions2D[length-1], positions2D[0], positions2D[1]);
        if (turnDirection * currentTurnDirection < 0) {
	        return false;
        }
        return true;
