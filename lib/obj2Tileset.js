    ...
    if (options.b3dm) {
        return obj2B3dm(objPath, options)
            .then(function(result){
                ...
                var width = minmaxPoint.max[x] - minmaxPoint.min[x];
                var height = minmaxPoint.max[y] - minmaxPoint.min[y];
                width = Math.ceil(width);
                height = Math.ceil(height);
                var offsetX = width / 2 + minmaxPoint.min[x];
                var offsetY = height / 2 + minmaxPoint.min[y];
