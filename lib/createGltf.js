...
function createGltf(objData, options) {
    ...
    return {
        gltf: gltf,
        batchTableJson: bufferState.batchTableJson
    };
}
...
function addBuffers(gltf, bufferState, name) {
    ...
    addBufferView(gltf, bufferState.batchIdBuffers, bufferState.batchIdAccessors, 0, WebGLConstants.ARRAY_BUFFER);

    var buffers = [];
    buffers = buffers.concat(bufferState.positionBuffers, bufferState.normalBuffers, bufferState.uvBuffers, bufferState.indexBuffers, bufferState.batchIdBuffers);
...
function addBacthIdAttribute(gltf, batchId, count, name) {
    var accessor = {
        name: name,
...
function addMesh(gltf, materials, bufferState, uint32Indices, mesh, options) {
    ...
    var useBatchId = options.batchId;
    ...
    if (hasPositions) {
        ...
        if (useBatchId) {
            bufferState.batchTableJson.maxPoint.push(gltf.accessors[accessorIndex].max);
            bufferState.batchTableJson.minPoint.push(gltf.accessors[accessorIndex].min);
        }
    }
    if (hasNormals) {
    ...
    if(useBatchId) {
        var batchIdCount = mesh.positions.length / 3;
        accessorIndex = addBacthIdAttribute(gltf, bufferState.currentBatchId, batchIdCount, mesh.name + '_batchId');
        attributes._BATCHID = accessorIndex;
        var batchIdArray = new ArrayStorage(ComponentDatatype.UNSIGNED_SHORT);
        for(var j = 0; j < batchIdCount; j++){
            batchIdArray.push(bufferState.currentBatchId);
        }
        bufferState.batchIdBuffers.push(batchIdArray.toUint16Buffer());
        bufferState.batchIdAccessors.push(accessorIndex);
        bufferState.batchTableJson.batchId.push(bufferState.currentBatchId);
        bufferState.batchTableJson.name.push(mesh.name.match(nameRegtex) ? mesh.name.match(nameRegtex)[0] : 'Unformat Name');
        bufferState.currentBatchId ++;
    }
